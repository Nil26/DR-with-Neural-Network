...
class DependencyRNN:
    ...
    def __init__(self, d, V, r, answer_idxs, embeddings=None, seed=0):
        ...
        # self.params = [self.We, self.Wr, self.Wv, self.b] # update all
        self.params = [self.Wr, self.Wv, self.b] # fixed word embeddings
        ...
        def normalized_tanh(x):
            ...
            tanh_vector = T.tanh(x)
            return tanh_vector/T.sqrt(T.dot(tanh_vector,tanh_vector))
        ...
        def recurrence(n, hidden_states, hidden_sums, cost, x, r, p, wrong_ans, corr_ans):
            ...
            cost - the total cost so far for this tree (for this one sentence)
            ...
            (in that order)
            '''
            # compute the value of hidden_states[n]
            new_states = T.set_subtensor(hidden_states[n], self.f(T.dot(self.Wv, x[n]) + self.b + hidden_sums[n])) 
            # update hidden_sums
            new_sums = T.inc_subtensor(hidden_sums[p[n]],  T.dot(r[n], new_states[n]))
            # update cost
            new_cost, updates = theano.scan(fn=lambda x_z, prev_cost, x_c, h_s: prev_cost + T.maximum(0., 1. - T.dot(x_c, h_s) + T.dot(x_z, h_s)),
                            sequences=wrong_ans,
                            outputs_info=cost,
                            non_sequences=[corr_ans, new_states[n]]
                            )

            return new_states, new_sums, new_cost[-1]
    ...
    def load(cls, filename):
        ...
        with open(filename) as f:
            ...
            # npzfile.keys() = ['Wv', 'dependencies', 'b', 'answer_idxs']
